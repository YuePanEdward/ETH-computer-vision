function matchingCost = shape_matching(X,Y,display_flag,nsamp)
...
while currentIteration <= maxIterations
   ...
   ShapeDescriptors1 = sc_compute(currentX',nbBins_theta,nbBins_r,smallest_r,biggest_r,nsamp);
   %disp('done.')
      
   %disp('computing shape contexts for target...')
   ShapeDescriptors2 = sc_compute(Y',nbBins_theta,nbBins_r,smallest_r,biggest_r,nsamp);
   %disp('done.')

   %set lambda here      
   lambda = mean2(sqrt(dist2(Y,Y))).^2;  
   
   %write the chi2cost.m function
   %disp('computing chi2 cost...')
   costMatrixC = chi2cost(ShapeDescriptors1,ShapeDescriptors2); 
   %disp('done.')

   %disp('computing correspondences index...')
