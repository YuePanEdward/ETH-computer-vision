...
% EXERCISE 5.1
dispStereoL = stereoDisparity(rgb2gray(imgRectL), rgb2gray(imgRectR), dispRange);
dispStereoR = stereoDisparity(rgb2gray(imgRectR), rgb2gray(imgRectL), dispRange);
...
% exercise 5.2
Labels = gcDisparity(rgb2gray(imgRectL), rgb2gray(imgRectR), dispRange);
dispsGCL = double(Labels + dispRange(1));
Labels = gcDisparity(rgb2gray(imgRectR), rgb2gray(imgRectL), dispRange);
...
coords = generatePointCloudFromDisps(dispsGCL, inv(S)*PL, inv(S)*PR);
% ... do same for other winner-takes-all
coords2 = generatePointCloudFromDisps(dispStereoL, inv(S)*PL, inv(S)* PR);
...
% ... do same for other winner-takes-all
generateObjFile('modelWTA', 'imgRectL.png', coords2, maskStereoL.*maskStereoR);