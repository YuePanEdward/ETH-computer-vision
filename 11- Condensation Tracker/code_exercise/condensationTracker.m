...
elseif(use_mov)
    vid = VideoReader(['../data/' videoName '.mov']);
else
...
%     figure(2);
%     plot(hist(1,:),'r','linewidth',1'); hold on; grid on;
%     plot(hist(2,:),'g','linewidth',1'); hold on;
%     plot(hist(3,:),'b','linewidth',1'); hold on;
%     title('Color Histograms')
%     xlabel('Bins')
%     ylabel('Counts in each bin')
